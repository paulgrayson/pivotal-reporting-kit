...
describe PRKit::CompositeApiResponse do
  ...
  describe '#error?' do
    ...
    it 'is ok if all responses are ok' do
      composite = PRKit::CompositeApiResponse.new([ok_response, ok_response])
      composite.error?.should eq false
    end

    it 'is an error if any responses are error' do
      composite = PRKit::CompositeApiResponse.new([ok_response, error_response, ok_response])
  ...
  describe '#data' do
    it 'concatenates data from each response' do
      ...
      composite = PRKit::CompositeApiResponse.new([response1, response2, response3])
